...
const KINDS = Object.keys(PIECE_CONFIG) as PieceKind[];
...
type Panel = HTMLElement & {
  ...
  grids: GridData[];
  pieces: PieceData[];
  prefabRefs: Record<PieceKind, { uuid: string; url: string }>;
  selectedKind: PieceKind;
  ...
  render(): void;
  syncPreview(): void;
  setStatus(message: string, type?: 'ok' | 'error'): void;
  place(kind: PieceKind, cell: Cell): void;
  loadDemo(): void;
};

function valueOf(element: any): number {
  return Math.max(1, Math.floor(Number(element.value) || 1));
}

function currentAxis(panel: Panel): RotationAxis {
  return panel.$.axis.value === 'X' ? 'X' : 'Z';
}

function currentAngle(panel: Panel): RotationAngle {
  return Number(panel.$.angle.value) as RotationAngle;
}

function selectedPiece(panel: Panel): PieceData | undefined {
  return panel.pieces.find((piece) => piece.id === panel.selectedPieceId);
}

function createDemo(panel: Panel): void {
  panel.grids = [0, 1, 2].map(() => ({ width: 5, height: 5, visible: true }));
  const ref = (kind: PieceKind) => panel.prefabRefs[kind] || { uuid: '', url: '' };
  panel.pieces = [
    { id: 1, kind: 'cube3', ...{ prefabUuid: ref('cube3').uuid, prefabUrl: ref('cube3').url }, axis: 'Z', angle: 90, anchor: { x: 0, y: 0, z: 0 } },
    { id: 2, kind: 'cube4', ...{ prefabUuid: ref('cube4').uuid, prefabUrl: ref('cube4').url }, axis: 'Z', angle: 0, anchor: { x: 1, y: 0, z: 0 } },
    { id: 3, kind: 'cube3', ...{ prefabUuid: ref('cube3').uuid, prefabUrl: ref('cube3').url }, axis: 'X', angle: 90, anchor: { x: 4, y: 4, z: 0 } },
    { id: 4, kind: 'cube1', ...{ prefabUuid: ref('cube1').uuid, prefabUrl: ref('cube1').url }, axis: 'Z', angle: 0, anchor: { x: 2, y: 2, z: 1 } },
    { id: 5, kind: 'special', ...{ prefabUuid: ref('special').uuid, prefabUrl: ref('special').url }, axis: 'Z', angle: 0, anchor: { x: 0, y: 4, z: 2 } },
  ];
...
module.exports = Editor.Panel.define({
  ...
  methods: {
    setStatus(this: Panel, message: string, type?: 'ok' | 'error'): void {
      this.$.status.textContent = message;
      this.$.status.className = `status ${type || ''}`;
    },

    syncPreview(this: Panel): void {
      ...
      this.syncTimer = window.setTimeout(async () => {
        try {
          ...
          this.setStatus(`Scene preview: ${(error as Error).message}`, 'error');
        }
      }, 30);
    },

    place(this: Panel, kind: PieceKind, anchor: Cell): void {
      const ref = this.prefabRefs[kind];
      if (!ref?.uuid) {
        this.setStatus(`Hãy kéo ${PIECE_CONFIG[kind].label} Prefab vào ô tương ứng trước.`, 'error');
        return;
      }
      const rotatable = PIECE_CONFIG[kind].rotatable;
      const piece: PieceData = {
        id: nextPieceId(this.pieces), kind, prefabUuid: ref.uuid, prefabUrl: ref.url,
        axis: rotatable ? currentAxis(this) : 'Z', angle: rotatable ? currentAngle(this) : 0, anchor,
      };
      const error = validatePiece(piece, this.grids, this.pieces);
      if (error) return this.setStatus(error, 'error');
      this.pieces.push(piece);
      this.selectedPieceId = piece.id;
      this.setStatus(`Đã đặt ${PIECE_CONFIG[kind].label} tại (${anchor.x}, ${anchor.y}, ${anchor.z}).`, 'ok');
      this.render();
    },

    loadDemo(this: Panel): void { createDemo(this); },

    render(this: Panel): void {
      ...
      if (!this.grids.length) this.$.grids.textContent = 'Chưa có Grid. Nhập Width/Height rồi bấm Create Grid.';

      this.grids.forEach((grid, layer) => {
        if (!grid.visible || (this.viewMode === 'focus' && layer !== this.focusedLayer)) return;
        ...
        for (let y = grid.height - 1; y >= 0; y -= 1) {
          for (let x = 0; x < grid.width; x += 1) {
            ...
            const piece = findPieceAt(this.pieces, cell);
            const element = document.createElement('button');
            element.className = `cell ${(x + y) % 2 ? 'checker' : ''}`;
            element.title = `(${x}, ${y}, ${layer})`;
            if (piece) {
              element.style.background = PIECE_CONFIG[piece.kind].color;
              if (piece.id === this.selectedPieceId) element.classList.add('selected-piece');
              if (piece.anchor.x === x && piece.anchor.y === y && piece.anchor.z === layer) element.classList.add('anchor');
            }
            if (!piece) element.textContent = `${x},${y}`;
            element.onclick = () => {
              if (piece) { this.selectedPieceId = piece.id; this.render(); }
              else this.place(this.selectedKind, cell);
            };
            element.oncontextmenu = (event) => {
              event.preventDefault();
              if (piece) { this.pieces = this.pieces.filter((item) => item.id !== piece.id); this.selectedPieceId = null; this.render(); }
            };
            ...
            element.ondrop = (event) => {
              ...
              const draggedKind = event.dataTransfer?.getData('application/x-level-piece') as PieceKind;
              if (!KINDS.includes(draggedKind)) return this.setStatus('Chỉ kéo Piece từ thanh Prefab của tool vào Grid.', 'error');
              if (piece) return this.setStatus(`Cell (${x}, ${y}, ${layer}) đã có Piece.`, 'error');
      ...
      this.$.selection.textContent = selected
        ? `${PIECE_CONFIG[selected.kind].label} #${selected.id} · Anchor (${selected.anchor.x}, ${selected.anchor.y}, ${selected.anchor.z}) · ${selected.axis} ${selected.angle}°`
        : `Đang chọn ${PIECE_CONFIG[this.selectedKind].label}. Click Cell để đặt; click Piece để chọn.`;
      this.$.rotate.disabled = !selected || !PIECE_CONFIG[selected.kind].rotatable;
      this.$.delete.disabled = !selected;
      this.syncPreview();
    },
  },

  ready(this: Panel): void {
    this.grids = [];
    this.pieces = [];
    this.prefabRefs = {} as Panel['prefabRefs'];
    ...
    for (const kind of KINDS) {
      ...
      option.innerHTML = `<strong><span class="swatch" style="background:${PIECE_CONFIG[kind].color}"></span>${PIECE_CONFIG[kind].label}</strong>`;
      const asset = document.createElement('ui-asset') as any;
      asset.setAttribute('droppable', 'cc.Prefab');
      asset.addEventListener('confirm', async (event: Event) => {
        const uuid = String((event.target as any).value || '');
        let url = '';
        if (uuid) {
        ...
        this.setStatus(uuid ? `Đã gán ${PIECE_CONFIG[kind].label}.` : `Đã bỏ gán ${PIECE_CONFIG[kind].label}.`, 'ok');
      });
      ...
      dragPiece.addEventListener('dragstart', (event) => {
        ...
        event.dataTransfer?.setData('application/x-level-piece', kind);
        event.dataTransfer?.setData('text/plain', kind);
        if (event.dataTransfer) event.dataTransfer.effectAllowed = 'copy';
      });
      option.appendChild(dragPiece);
      option.addEventListener('click', (event) => {
        if ((event.target as HTMLElement).tagName.toLowerCase() === 'ui-asset') return;
        this.selectedKind = kind;
        this.$.prefabs.querySelectorAll('.piece-option').forEach((el: HTMLElement) => el.classList.toggle('selected', el.dataset.kind === kind));
        this.render();
      });
    ...
    this.$.deleteGrid.onclick = () => {
      ...
      const impacted = this.pieces.filter((piece) => occupiedCells(piece).some((cell) => cell.z === layer));
      ...
      const result = removeGrid(this.grids, this.pieces, layer);
    ...
    this.$.rotate.onclick = () => {
      ...
      if (!piece || !PIECE_CONFIG[piece.kind].rotatable) return;
      const candidate = { ...piece, axis: currentAxis(this), angle: currentAngle(this) };
      const error = validatePiece(candidate, this.grids, this.pieces, piece.id);
      if (error) return this.setStatus(error, 'error');
      piece.axis = candidate.axis;
      piece.angle = candidate.angle;
      this.setStatus(`Đã rotate ${PIECE_CONFIG[piece.kind].label}: ${piece.axis} ${piece.angle}°.`, 'ok');
      this.render();
    };
    this.$.clear.onclick = () => {
      this.pieces = [];
    ...
    this.$.generate.onclick = async () => {
      try {
        if (!this.pieces.length) throw new Error('Level chưa có Piece nào.');
        for (const piece of this.pieces) {
          const error = validatePiece(piece, this.grids, this.pieces, piece.id);
