...
assert.match(validatePiece(piece({ anchor: { x: 4, y: 1, z: 0 } }), grids, []), /vượt khỏi/);
assert.match(validatePiece(piece({ axis: 'X', angle: 90, anchor: { x: 1, y: 1, z: 1 } }), grids, []), /số layer/);

const existing = piece();
assert.match(validatePiece(piece({ id: 2, anchor: { x: 3, y: 1, z: 0 } }), grids, [existing]), /đã có Piece/);
...
assert.strictEqual(findPieceAt([existing], { x: 2, y: 1, z: 0 }).id, 1);
assert.strictEqual(findPieceAt([existing], { x: 2, y: 2, z: 0 }), undefined);
...
console.log('Level Editor model: 17 checks passed.');
